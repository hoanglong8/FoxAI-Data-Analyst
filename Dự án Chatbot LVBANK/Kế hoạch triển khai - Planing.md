...
- [ ] Chuẩn bị 1 domain trên server của FoxAI (ví dụ: `fox.ai.vn/chatbot_lvbank` dùng để thực thi lệnh call API ChatGPT từ file api.py), giao diện này có thể tham khảo của [BIDV](https://bidv.com.vn/) hoặc [Vietinbank](https://www.vietinbank.vn/) Người dùng lần đầu truy cập thì cần khai báo `Tên` và `SĐT` lưu trên local storage => Mr.Tấn phụ trách.
...
- [ ] Sau khi có domain, Mr.Ngọc đưa các file source code lên server.
...
- [ ] Tự động lưu trữ lịch sử chat theo ID của người dùng về server (theo dạng response `.json`) để tiếp tục huấn luyện, tăng cá nhân hóa => Mr.Tấn nghiên cứu DB để tự động lưu trữ trên server.
